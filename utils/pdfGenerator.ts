...
export const testLogoLoading = async (): Promise<boolean> => {
  try {
    ...
    if (logoAsset.localUri) {
      const base64 = await FileSystem.readAsStringAsync(logoAsset.localUri, { encoding: 'base64' });
      console.log('Logo base64 loaded:', base64 ? 'yes' : 'no');
      return !!base64;
  ...
  // Exported function to generate PDF, taking payload as parameter
  export const generatePdfReport = async (payload: ReportPayload): Promise<string> => {
    ...
    try {
      ...
      if (logoAsset.localUri) {
        ...
        console.warn('Logo asset localUri is missing:', logoAsset);
      }
    } catch (e) {
...
<html>
  ...
  <body>
    ...
    <table style="width:100%; border:1px solid #000; margin-bottom:20px; border-collapse: collapse;">
      ...
      <tr>
        <td style="padding:5px;"><label>Scheduled Time:</label> <span>${clientData.scheduledTime || '09:00'}</span></td>
<td style="padding:5px;"><label>Arrival Time:</label> <span>${clientData.arrivalTime || '09:00'}</span></td>
<td style="padding:5px;"><label>Departure Time:</label> <span>${clientData.departureTime || '17:00'}</span></td>
      </tr>
    </table>
